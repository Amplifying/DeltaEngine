	...
	public static class AppInfoExtensions
	{
		...
		private static MessageBoxResult ShowDownloadAndroidDriversQuestionAndGetUserDecision()
		{
			...
			string messageText = "There was currently no Android device found." + newLine +
				...
				newLine +
				newLine +
				"If the driver for your Android device is missing, click yes to download it automatically. " +
				"The folder with the driver and an instruction manual (pdf) will open to help you with the" +
				" installation.";
		...
		private static void TryDownloadAndroidDriverAndShowInstructions()
		{
			try
			{
				...
				TryOpenAndroidDriverInstallationManual(driverDownloader);
			}
			catch (Exception ex)
			{
